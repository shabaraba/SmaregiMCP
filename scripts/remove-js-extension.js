...
function removeJsExtension(filePath) {
  ...
  try {
    ...
    const relativeImportRegex = /import\s+(?:[\w\s{},*]+\s+from\s+)?['"](\.[^'"]+)\.js['"]/g;
    ...
    // SDK関連のimport文を修正
    const sdkImportRegex = /import\s+(?:[\w\s{},*]+\s+from\s+)?['"]@modelcontextprotocol\/sdk\/([^'"]+)\.js['"]/g;
    content = content.replace(sdkImportRegex, (match, importPath) => {
      modified = true;
      return match.replace(`${importPath}.js`, importPath);
    });
