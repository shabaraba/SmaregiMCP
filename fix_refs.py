#!/usr/bin/env python3
"""
Script to convert $ref paths to start with './' in OpenAPI YAML files.
This script updates all $ref paths that refer to YAML files in the schemas or paths directories
to have the './' prefix if they don't already have it.
"""
...
def fix_refs_in_file(file_path):
    """Update $ref paths in a single file to start with './'."""
    ...
    # Pattern to match $ref values that use relative paths but don't start with ./
    # This looks for $ref: '../../path/to/file.yaml' patterns and updates them to $ref: './../../path/to/file.yaml'
    pattern = r"(\$ref:\s*['\"])(\.\.\/.*?\.yaml)(['\"])"
    replacement = r"\1./\2\3"
    
    # Replace relative paths to add ./ prefix
...
def main():
    ...
    root_updated = 1 if fix_refs_in_file(root_file) else 0
